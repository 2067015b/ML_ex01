...
def get_predictions_SGD_l1(X_train, y_train, X_test, learning_rate=0.005, iterations=40500):
    X_train = np.concatenate((np.ones((X_train.shape[0],1)),X_train),axis=1)
    X_test = np.concatenate((np.ones((X_test.shape[0],1)),X_test),axis=1)
...
def score(X_val, y_val, X_train, y_train):
    return 1 - sum((get_predictions_SGD_l1(X_train,y_train, X_val) - y_val)**2) / sum((y_val - np.mean(y_val))**2)
