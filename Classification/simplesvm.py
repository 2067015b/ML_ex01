    ...
    def smo_optimise(self):
        ...
        while passes < self.max_passes:
            ...
            for i in range(self.N):
                ...
                if (self.traint[i] * Ei < -self.tol and self.alpha[i] < self.C) or (
                            self.traint[i] * Ei > self.tol and self.alpha[i] > 0):
                    ...
                    b1 = self.b - Ei - self.traint[i] * (self.alpha[i] - alphai) * self.K[i, i] - self.traint[j] * (
                    self.alpha[j] - alphaj) * self.K[i, j]
                    b2 = self.b - Ej - self.traint[i] * (self.alpha[i] - alphai) * self.K[i, j] - self.traint[j] * (
                    self.alpha[j] - alphaj) * self.K[j, j]
...
if __name__ == "__main__":
    ...
    y_train -= 1
    y_train = np.reshape(y_train,(600,1))

    X_train, X_test = normalize_data(X_train,X_test)

    svm = SimpleSVM(X_train, y_train, kernel='rbf', C=100)
    svm.smo_optimise()

    y_pred = []
    for row in X_test:
        y_pred.append(svm.test_predict(row))
